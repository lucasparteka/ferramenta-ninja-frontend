import { formatCardNumber, type GeneratedCard } from '@/lib/credit-card/generate'
    ...
    >
      <div className="flex h-full flex-col justify-between">
        <div className="flex items-start justify-between">
          ...
          <span className={`text-sm font-semibold tracking-wide ${textColor} opacity-90`}>
            {card.brandLabel}
          </span>
        </div>

        <div className={`font-mono text-xl font-bold tracking-widest ${textColor}`}>
          {formatted}
        </div>

        <div className="flex items-end justify-between">
          <div>
            <p className={`text-xs uppercase tracking-widest ${textColor} opacity-60`}>
              Validade
            </p>
            <p className={`font-mono text-sm font-semibold ${textColor}`}>{card.expiry}</p>
          </div>
          <div className="text-right">
            <p className={`text-xs uppercase tracking-widest ${textColor} opacity-60`}>CVV</p>
            <p className={`font-mono text-sm font-semibold ${textColor}`}>{card.cvv}</p>
          </div>
        </div>
      </div>
    </div>
