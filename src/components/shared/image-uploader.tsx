...
export default function ImageUploader({
  ...
  const onSelectFile = (file?: File) => {
    ...
    if (file.size > maxFileSizeBytes) {
      setError(`Arquivo muito grande. Máximo ${maxFileSizeBytes / (1024 * 1024)}MB.`);
      return;
    }
    ...
    <div>
      ...
      <div className="flex items-center gap-4">
        <div className="relative w-24 h-24 shrink-0 border rounded-md overflow-hidden flex items-center justify-center">
          {preview ? (
            ...
            <span className="text-xs text-muted-foreground text-center px-1 border-dashed">Sem imagem</span>
          )}
        </div>
      ...
      <div className="mt-3">
        ...
        <p className="text-xs text-muted-foreground">
          Tamanho máximo {maxFileSizeBytes / (1024 * 1024)}MB. Extensões aceitas: PNG, JPG.
        </p>
      </div>
