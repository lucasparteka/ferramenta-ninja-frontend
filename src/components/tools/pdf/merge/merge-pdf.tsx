...
export function MergePDF() {
  ...
  async function processFiles(selected: FileList | File[]) {
    const valid = Array.from(selected).filter((f) => f.type === 'application/pdf')
    if (valid.length === 0) return

    const startIndex = files.length

    setFiles((prev) => [...prev, ...valid])
    setThumbnails((prev) => [...prev, ...valid.map(() => null as string | null)])
    setResult(null)
    setErrorMsg('')
  ...
  async function handleMerge() {
    ...
    try {
      ...
      setState('error')
      setErrorMsg('Erro ao juntar os PDFs. Verifique se todos os arquivos são PDFs válidos.')
    }
  }
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="space-y-1">
        <label className="block text-sm font-medium text-foreground">Adicionar PDFs</label>
        <div
          role="button"
          tabIndex={0}
        ...
        >
          ...
          <p className="text-xs text-muted-foreground">Vários arquivos permitidos</p>
        </div>
        <input
          ref={inputRef}
          type="file"
