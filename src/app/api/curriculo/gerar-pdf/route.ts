...
export async function POST(request: Request) {
  ...
  if (!formData || !layout) {
    return NextResponse.json({ error: "Dados do currículo ausentes" }, { status: 400 })
  }

  const token = crypto.randomUUID()
  setTempData(token, { formData, layout, photoDataUrl })

  const baseUrl = process.env.INTERNAL_URL ?? process.env.NEXT_PUBLIC_SITE_URL ?? "http://localhost:3000"
