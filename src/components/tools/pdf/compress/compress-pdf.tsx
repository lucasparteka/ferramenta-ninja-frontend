...
const COMPRESSION_LEVELS: { value: CompressionLevel; label: string }[] = [
  ...
  { value: 'medio', label: 'Médio — equilíbrio entre tamanho e compatibilidade' },
  { value: 'alto', label: 'Alto — recompressão de imagens no servidor (maior redução)' },
]
  ...
  async function handleCompress() {
    ...
    try {
      ...
      if (level === 'alto') {
        ...
        const res = await fetch('/api/comprimir-pdf', { method: 'POST', body: form })
      ...
      setState('error')
      setErrorMsg('Erro ao comprimir o PDF. Verifique se o arquivo é um PDF válido.')
    }
  }
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="space-y-1">
        ...
        <p className="text-xs text-muted-foreground">
          A compressão no navegador remove objetos não utilizados e otimiza a estrutura do
          arquivo. Para PDFs com muitas imagens, a redução pode ser limitada.
        </p>
      </div>
      ...
      {state === 'done' && result && (
        ...
        >
          <div>
            ...
            <p className="mt-1 font-semibold text-foreground">{formatBytes(originalSize)}</p>
          </div>
          <div>
