...
function SectionHeader({ title }: { title: string }) {
	return (
		<div className="mb-3">
			...
			<div
				className="h-px mt-1.5 opacity-30"
...
export function ClassicTemplate({
	...
	return (
		...
		>
			<div
				className="px-10 pt-6 pb-7 flex items-start justify-between gap-6 border-b-4"
			...
			>
				<div className="flex-1 min-w-0">
					...
					{data.headline && <p className="text-base mt-1">{data.headline}</p>}
						...
						<div className="space-y-4">
							{data.education?.map((edu, i) => (
								<div key={edu.degree + edu.startYear} className="pdf-item">
									...
									{edu.description && (
										<p className="text-base text-[#444] mt-1.5 leading-[1.6]">
											{edu.description}
										</p>
									)}
