  ...
  for (let row = 0; row < rows; row++) {
    for (let col = 0; col < cols; col++) {
      ...
      if (labeled) {
        ...
        ctx.fillText(String(rendered + 1), x + stampSize / 2, y + stampSize / 2 + 3)
        ctx.textAlign = 'left'
        ctx.globalAlpha = 1
      }
...
type LayoutFn = (ctx: CanvasRenderingContext2D, data: BackData, w: number, h: number) => void
...
const BACK_LAYOUTS: Record<BackLayout, LayoutFn> = {
  ...
  elegante(ctx, data, w, h) {
    ...
    ctx.arcTo(frameX + frameW, frameY + frameH, frameX + frameW - fr, frameY + frameH, fr)
  ...
  minimalista(ctx, data, w, h) {
    ...
    drawStamps(ctx, data, 0, stampAreaY, w, h - stampAreaY - (data.rulesText ? 22 : 10))
  ...
  negrito(ctx, data, w, h) {
    ...
    ctx.font = '9px Inter, sans-serif'
    ctx.textAlign = 'center'
    ctx.fillText(data.rulesText || `${data.stampCount} carimbos`, (splitX + w) / 2, h - 12)
    ctx.globalAlpha = 1
    ctx.textAlign = 'left'
  },
