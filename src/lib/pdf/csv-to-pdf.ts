...
function chooseDimensions(columnCount: number): {
	pageW: number;
	pageH: number;
} {
...
function truncateText(
	...
	while (
		result.length > 0 &&
		safeTextWidth(`${result}\u2026`, font, size) > maxWidth
	) {
...
function getIdealColumnWidths(
	...
	return headers.map((header, colIndex) => {
		const headerW =
			safeTextWidth(header || `Col ${colIndex + 1}`, boldFont, fontSize) +
			PADDING_H * 2;
		let maxCellW = 0;
...
export async function generateCsvPdf(data: CsvData): Promise<Uint8Array> {
	...
	const idealWidths = getIdealColumnWidths(
		headers,
		rows,
		font,
		boldFont,
	...
	for (const group of groups) {
		...
		const groupRows = rows.map((r) =>
			r.slice(group.startIndex, group.endIndex),
		);
		const groupIdealWidths = idealWidths.slice(
			group.startIndex,
			group.endIndex,
		);
		const colWidths =
