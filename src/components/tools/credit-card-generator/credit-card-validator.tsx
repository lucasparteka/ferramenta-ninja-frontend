...
import { validateCreditCard, type ValidationResult } from '@/lib/credit-card/validate'
  ...
  return (
    <div className="space-y-4">
      <div className="space-y-2">
        <label htmlFor="card-number-input" className="text-sm font-medium text-foreground">
          Número do cartão
        </label>
        <input
      ...
      {result && (
        ...
        >
          ...
          {result.formatted && (
            <p className="mt-1 font-mono text-sm opacity-80">{result.formatted}</p>
          )}
          <p className="mt-1 text-sm opacity-80">Bandeira detectada: {result.brand}</p>
          {!result.valid && (
