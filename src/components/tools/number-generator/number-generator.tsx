...
export function NumberGenerator() {
  const [options, setOptions] = useState<NumberGeneratorOptions>(DEFAULT_OPTIONS)
  const [numbers, setNumbers] = useState<number[]>([])
  const [copied, setCopied] = useState(false)

  function set<K extends keyof NumberGeneratorOptions>(key: K, value: NumberGeneratorOptions[K]) {
      ...
      <div className="space-y-6 sm:w-[30%] sm:shrink-0">
        <div className="grid grid-cols-1 gap-4">
          <div className="space-y-1">
            <label htmlFor="num-count" className="flex w-full text-sm font-medium text-foreground">
              Números
            </label>
            <input
              id="num-count"
          ...
          <div className="space-y-1">
            <label className="flex w-full text-sm font-medium text-foreground">Entre</label>
          ...
          <div className="space-y-1">
            <label htmlFor="num-columns" className="flex w-full text-sm font-medium text-foreground">
              Colunas
            </label>
            <select
              id="num-columns"
          ...
          <div className="space-y-1">
            <label htmlFor="num-order" className="flex w-full text-sm font-medium text-foreground">
              Ordem
            </label>
            <select
              id="num-order"
          ...
          <div className="space-y-2">
            <span className="text-sm font-medium text-foreground">Números únicos</span>
        ...
        {uniqueWarning && (
          <p className="text-sm text-warning">
            O intervalo contém {rangeSize} números únicos disponíveis. A quantidade será ajustada
            automaticamente.
          </p>
        )}
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <p className="text-sm font-medium text-foreground">
                {numbers.length} {numbers.length === 1 ? 'número gerado' : 'números gerados'}
              </p>
