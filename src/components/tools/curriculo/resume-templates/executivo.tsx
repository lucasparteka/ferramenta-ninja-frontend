	...
	return (
		...
		>
			<div
				...
				style={{
					backgroundColor: "color-mix(in srgb, var(--accent) 18%, #1a2a38)",
				}}
			>
				{data.photoUrl ? (
				...
				<div className="flex-1 min-w-0 text-white">
					...
					{data.headline && (
						<p className="text-base mt-1 opacity-90">{data.headline}</p>
					)}
			...
			<div className="px-9 py-7 space-y-8">
											...
											<ul className="mt-2 space-y-1.5">
												{exp.description
													...
													.map((line, j) => (
														<li
															key={j}
															className="text-sm leading-relaxed flex gap-2 opacity-90 pl-4"
														>
							...
							{data.education!.map((edu, i) => (
								<div key={i} className="pdf-item flex gap-5">
									...
									>
										...
										<p
											className="text-sm font-semibold mt-0.5 opacity-80"
											style={{ color: "var(--accent)" }}
										>
											{edu.institution}
										</p>
				...
				{(hasSkills || hasLanguages) && (
					<div
						className={
							hasSkills && hasLanguages ? "grid grid-cols-2 gap-8" : ""
						}
					>
						{hasSkills && (
							<section className="section">
