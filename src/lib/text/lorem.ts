const LOREM_WORDS = [
  ...
  'accusantium', 'doloremque', 'laudantium', 'totam', 'aperiam', 'eaque', 'illo',
  'inventore', 'veritatis', 'quasi', 'architecto', 'beatae', 'vitae', 'dicta',
  'explicabo', 'ipsam', 'voluptas', 'aspernatur', 'odit', 'consequuntur', 'magni',
  'dolores', 'ratione', 'sequi', 'nesciunt', 'neque', 'porro', 'quisquam', 'blanditiis',
  'praesentium', 'voluptatum', 'deleniti', 'corrupti', 'mollitia', 'provident',
  'similique', 'possimus', 'assumenda', 'repellendus', 'temporibus', 'quibusdam',
  'officiis', 'debitis', 'rerum', 'necessitatibus', 'saepe', 'eveniet', 'voluptatibus',
  'repudiandae', 'recusandae', 'itaque', 'earum', 'facilis', 'expedita', 'distinctio',
]
...
export function generateLoremWords(count: number, startWithLorem: boolean): string {
  if (startWithLorem) {
    const startWords = LOREM_START.replace(',', '').toLowerCase().split(' ')
    const extra = Array.from({ length: Math.max(0, count - startWords.length) }, randomLoremWord)
    return [...startWords, ...extra].slice(0, count).join(' ')
  }
  return Array.from({ length: count }, randomLoremWord).join(' ')
}

export function generateLoremSentences(count: number, startWithLorem: boolean): string {
...
export function generateLoremParagraphs(count: number, startWithLorem: boolean): string {
  return Array.from({ length: count }, (_, pIndex) => {
    ...
    return Array.from({ length: sentenceCount }, (__, sIndex) => {
      if (pIndex === 0 && sIndex === 0 && startWithLorem) return LOREM_START + '.'
      const wordCount = Math.floor(Math.random() * 8) + 6
      return buildSentence(Array.from({ length: wordCount }, randomLoremWord))
    }).join(' ')
  }).join('\n\n')
