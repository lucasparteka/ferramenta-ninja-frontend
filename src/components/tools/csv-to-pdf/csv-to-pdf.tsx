...
export function CsvToPdf() {
  ...
  async function handleDownload() {
    ...
    setState('generating')
    try {
      const bytes = await generateCsvPdf({ headers: parsed.headers, rows: parsed.rows })
  ...
  const pageEnd = parsed ? Math.min(pageStart + ROWS_PER_PAGE, parsed.rows.length) : 0
  ...
  return (
    <div className="space-y-4">
      ...
      {state !== 'preview' && state !== 'generating' && (
        <Button onClick={handleParse} disabled={!hasContent || isBusy} className="w-full">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <div className="flex items-center gap-2 rounded-md border border-border bg-secondary px-3 py-1.5 text-sm text-muted-foreground">
              ...
              <span>
                ...
                <span className="font-medium text-foreground">{parsed.headers.length}</span>{' '}
              ...
              <thead>
                <tr>
                  {parsed.headers.map((header, index) => (
                    ...
                    >
                      <span className="block max-w-[180px] truncate" title={header}>
