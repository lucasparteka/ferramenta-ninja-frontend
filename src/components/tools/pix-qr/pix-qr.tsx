...
export function PixQR() {
  ...
  async function handleGenerate() {
    setError('')
    if (!key.trim()) { setError('Informe a chave Pix.'); return }
    if (!name.trim()) { setError('Informe o nome do beneficiário.'); return }
    if (!city.trim()) { setError('Informe a cidade do beneficiário.'); return }
    ...
    try {
      ...
      const url = await generateQRCode({ text: payload, size: 300, errorCorrectionLevel: 'M' })
      setDataUrl(url)
    } catch {
    ...
    <div className="flex flex-col gap-6 sm:flex-row">
      <div className="space-y-4 sm:w-[30%] sm:shrink-0">
        <div className="space-y-1">
          <label htmlFor="pix-key-type" className="flex w-full text-sm font-medium text-foreground">
            Tipo de chave
          </label>
          <select
            id="pix-key-type"
            value={keyType}
            onChange={(e) => { setKeyType(e.target.value as PixKeyType); setKey(''); resetQR() }}
        ...
        <div className="space-y-1">
          <label htmlFor="pix-key" className="flex w-full text-sm font-medium text-foreground">
            Chave Pix
          </label>
          <input
            id="pix-key"
            ...
            value={key}
            onChange={(e) => { setKey(e.target.value); resetQR() }}
            className={inputClass}
          />
        </div>

        <div className="space-y-1">
          <label htmlFor="pix-name" className="flex w-full text-sm font-medium text-foreground">
          ...
          <input
            ...
            maxLength={25}
            value={name}
            onChange={(e) => { setName(e.target.value); resetQR() }}
            className={inputClass}
          />
        </div>

        <div className="space-y-1">
          <label htmlFor="pix-city" className="flex w-full text-sm font-medium text-foreground">
            Cidade
          </label>
          <input
            id="pix-city"
            ...
            maxLength={15}
            value={city}
            onChange={(e) => { setCity(e.target.value); resetQR() }}
            className={inputClass}
          />
        </div>

        <div className="space-y-1">
          <label htmlFor="pix-amount" className="flex w-full text-sm font-medium text-foreground">
            Valor (opcional)
          </label>
          <input
            id="pix-amount"
            ...
            placeholder="0.00"
            value={amount}
            onChange={(e) => { setAmount(e.target.value); resetQR() }}
            className={inputClass}
          />
        </div>

        <div className="space-y-1">
          <label htmlFor="pix-desc" className="flex w-full text-sm font-medium text-foreground">
            Descrição (opcional)
          </label>
          <input
            id="pix-desc"
            ...
            maxLength={72}
            value={description}
            onChange={(e) => { setDescription(e.target.value); resetQR() }}
            className={inputClass}
          />
        </div>
