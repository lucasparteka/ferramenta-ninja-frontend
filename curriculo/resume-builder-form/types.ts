...
const experienceSchema = z
  .object({
    ...
    startDate: z.string().min(7, "Data de início é obrigatória"), // "YYYY-MM"
    endDate: z.string().default(""),
    isCurrent: z.boolean().default(false),
    description: z.string().default(""),
  })
  .superRefine((val, ctx) => {
    if (!val.isCurrent && val.endDate && val.startDate && val.endDate < val.startDate) {
      ctx.addIssue({
...
const educationSchema = z
  .object({
    ...
    startYear: z.string().min(4, "Ano de início é obrigatório"), // "YYYY"
    endYear: z.string().default(""),
    isCurrent: z.boolean().default(false),
    description: z.string().default(""),
  })
  .superRefine((val, ctx) => {
    if (!val.isCurrent && val.endYear && val.startYear && val.endYear < val.startYear) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
...
export const resumeFormSchema = z.object({
  // Personal info — maps to resumes fields
  ...
  // Resume metadata — maps to resumes fields
  title: z.string().optional().default(""),
  summary: z.string().default(""),

  // Structured lists stored as JSON in PocketBase
